// Code generated by mockery v2.26.1. DO NOT EDIT.

package mocks

import (
	constants "scheduler0/constants"
	db "scheduler0/db"

	hclog "github.com/hashicorp/go-hclog"

	mock "github.com/stretchr/testify/mock"

	models "scheduler0/models"

	raft "github.com/hashicorp/raft"
)

// Scheduler0RaftActions is an autogenerated mock type for the Scheduler0RaftActions type
type Scheduler0RaftActions struct {
	mock.Mock
}

// ApplyRaftLog provides a mock function with given fields: logger, l, _a2, useQueues, queue, stopAllJobsQueue, recoverJobsQueue
func (_m *Scheduler0RaftActions) ApplyRaftLog(logger hclog.Logger, l *raft.Log, _a2 db.DataStore, useQueues bool, queue chan []interface{}, stopAllJobsQueue chan bool, recoverJobsQueue chan bool) interface{} {
	ret := _m.Called(logger, l, _a2, useQueues, queue, stopAllJobsQueue, recoverJobsQueue)

	var r0 interface{}
	if rf, ok := ret.Get(0).(func(hclog.Logger, *raft.Log, db.DataStore, bool, chan []interface{}, chan bool, chan bool) interface{}); ok {
		r0 = rf(logger, l, _a2, useQueues, queue, stopAllJobsQueue, recoverJobsQueue)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(interface{})
		}
	}

	return r0
}

// WriteCommandToRaftLog provides a mock function with given fields: rft, commandType, sqlString, nodeId, params
func (_m *Scheduler0RaftActions) WriteCommandToRaftLog(rft *raft.Raft, commandType constants.Command, sqlString string, nodeId uint64, params []interface{}) (*models.Response, error) {
	ret := _m.Called(rft, commandType, sqlString, nodeId, params)

	var r0 *models.Response
	var r1 error
	if rf, ok := ret.Get(0).(func(*raft.Raft, constants.Command, string, uint64, []interface{}) (*models.Response, error)); ok {
		return rf(rft, commandType, sqlString, nodeId, params)
	}
	if rf, ok := ret.Get(0).(func(*raft.Raft, constants.Command, string, uint64, []interface{}) *models.Response); ok {
		r0 = rf(rft, commandType, sqlString, nodeId, params)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*models.Response)
		}
	}

	if rf, ok := ret.Get(1).(func(*raft.Raft, constants.Command, string, uint64, []interface{}) error); ok {
		r1 = rf(rft, commandType, sqlString, nodeId, params)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

type mockConstructorTestingTNewScheduler0RaftActions interface {
	mock.TestingT
	Cleanup(func())
}

// NewScheduler0RaftActions creates a new instance of Scheduler0RaftActions. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewScheduler0RaftActions(t mockConstructorTestingTNewScheduler0RaftActions) *Scheduler0RaftActions {
	mock := &Scheduler0RaftActions{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
